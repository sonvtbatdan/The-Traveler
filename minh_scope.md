...
The processed, aligned frames are on disk at:
C:\Users\minhv\Documents\The-Stream\assets\beam references\Gauss_orb_files_2\Gauss_cannon_explosion

THREE explosion variants, each 12 frames, ALL aligned to the SAME spec so they're interchangeable: 394×404 each, burst core centered at (173,183), additive, transparent PNG. The files are named by variant + frame index (e.g. v1_00..v1_11, v2_00..v2_11, v3_00..v3_11). In STAGE 0, list that folder and report the EXACT filenames found, so the importer references real names (don't assume — confirm the naming/case).

Godot can only load from res://, so copy them into the project. Import to:
...
List the art folder C:\Users\minhv\Documents\The-Stream\assets\beam references\Gauss_orb_files_2\Gauss_cannon_explosion and report the EXACT filenames (so the importer uses real names/case). Confirm 3 variants × 12 frames, each 394×404 with alpha.
Locate the Gauss weapon. Report the file + fire-type handling (likely a gauss/charge fire_type in weapon_system.gd or an arena weapon node). Quote: how the orb is spawned, how it travels, and how it currently detects hitting a target / currently deals damage.
Report how OTHER AoE/tick damage is done in the codebase (e.g. acid_cloud, aura, rift zone) — there's likely an existing "damage all enemies in radius each interval" pattern to reuse. Quote it. Report how enemies are enumerated and damaged (the damage call signature, crit handling) and how get_weapon_stat/affixes apply so the 5/tick scales correctly.
Report the orb's current radius/size so GAUSS_EXPL_RADIUS (~2.4× it) and sprite scale can be set in real units.
Report how transient timed visuals are spawned/cleaned (the _tick_fx/transient pattern used elsewhere) so the explosion can be a self-expiring node/entry.
Propose the structure: on orb-hit → spawn explosion (sprite animator + DoT field + 2s lifetime) → per-frame advance the animation by the intro/peak-loop/outro schedule → every 0.1s damage enemies in GAUSS_EXPL_RADIUS → despawn at 2s.
