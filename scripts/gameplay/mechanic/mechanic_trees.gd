extends Node2D
class_name MechanicTrees
## Manages the Mechanic map's scattered 3D assets (one per .glb in assets/map/mechanic/ — see
## MechanicAssetScan). Port of electric_trees.gd's fully generic scatter/compositor system (see that file's
## header for the complete rationale — shared World3D "host" viewport, two-pass color+blur-mask render,
## orthogonal tilted camera), INCLUDING spawn_landmark() (for a future landmark spawner, unused today — see
## below). Based on volcanic_trees.gd's copy specifically (not Electric's directly), which already made the
## one deliberate omission this map wants too: no real 3D cloud-occluder plane — Mechanic's clouds are plain
## 2D parallax (mechanic_clouds.gd), not a horizontal sky band any tall asset needs to poke through/under.
##
## assets/map/mechanic/ currently has NO .glb files at all (only enemy sprite PNGs and the maptile canopy
## photos), so _types stays empty and the regular density-scatter places nothing — same "drop a .glb, no code
## changes" convention Electric/Volcanic already document, just with an empty pool for now.
...
const MechanicAssetLayerScript := preload("res://scripts/gameplay/mechanic/mechanic_asset_layer.gd")
const BLUR_MASK_SHADER := "res://scripts/gameplay/mechanic/mechanic_blur_mask.gdshader"
...
const MARGIN := 140.0
const CLOUD_ALTITUDE_PX := MechanicConfig.CLOUD_MAX_PX   # informational only (Terrain Edit panel's Height
                                                           # readout) — no occluder mesh clips against this
const TILT_MAX_DEG := 8.0

var _host_vp: SubViewport
var _world3d_root: Node3D
var _asset_layer: MechanicAssetLayer
...
var _jitter: float = MechanicTerrainSettings.DEFAULT_JITTER
var _cell_size: float = CELL_SIZE_MAX

func _ready() -> void:
	add_to_group("mechanic_trees")   # so the Terrain Edit panel can find this instance
	_z_comp = 1.0 / cos(deg_to_rad(MechanicAssetLayerScript.CAM_ISO_DEG))
	_build_host()
	await get_tree().process_frame
	await _measure_types()

	_asset_layer = MechanicAssetLayerScript.new()
	...
	for t: Dictionary in _types:
		...
		_settings_by_type[t["name"]] = MechanicTerrainSettings.asset_entry(s, t["name"])
...
## Public: keeps scattered assets off the SAME river band mechanic_ground.gdshader paints (MechanicNoise.
...
## node's own _ready() (persisted settings) — MechanicNoise.is_river()'s seam-based river check depends on the
## CURRENT mottle scale, same as mechanic_plumes.gd's own tracking of it for ground_region().
...
## _ready() (persisted settings) — how many of the 6 canopy-blend seams carry a river, see MechanicNoise.
## is_river()'s header.
...
func _measure_types() -> void:
	for glb_path: String in MechanicAssetScan.glb_paths():
...
func _measure_one(glb_path: String) -> Dictionary:
	...
	await get_tree().process_frame
	var aabb := MechanicAssetScan.combined_aabb(probe)
	...
	probe.queue_free()
	return {
		"name": MechanicAssetScan.type_name(glb_path),
		"packed": packed,
...
## system entirely — for a future one-off landmark object, unused today (Mechanic has no landmark .glb yet).
## Verbatim port of electric_trees.gd's spawn_landmark() — see that file for the full rationale.
func spawn_landmark(glb_path: String, world_pos: Vector2, scale_mult: float = 1.0) -> Dictionary:
	...
	MechanicAssetScan.set_visual_layers(outer, MechanicAssetLayerScript.COLOR_BIT)
...
func _regenerate(center: Vector2, force_full: bool = false) -> void:
	...
	for t: Dictionary in _types:
		var s: Dictionary = _settings_by_type.get(t["name"], MechanicTerrainSettings.default_asset_entry())
...
func _maybe_place(cell: Vector2) -> void:
	if _types.is_empty():
		return
	var start_i: int = int(MechanicNoise.hash21(cell + Vector2(17.0, 17.0)) * _types.size())
	for k in _types.size():
		var i: int = (start_i + k) % _types.size()
		var type_def: Dictionary = _types[i]
		var settings: Dictionary = _settings_by_type.get(type_def["name"], MechanicTerrainSettings.default_asset_entry())
		...
		var roll: float = MechanicNoise.hash21(cell * 1.271 + Vector2(300.0 + float(i) * 47.0, 900.0 + float(i) * 91.0))
		if roll >= chance:
			continue
		if int(_count_by_type[i]) >= MAX_INSTANCES_PER_TYPE:
			continue
		var jitter := Vector2(
			(MechanicNoise.hash21(cell + Vector2(23.0 + float(i) * 13.0, 0.0)) - 0.5) * _cell_size * _jitter,
			(MechanicNoise.hash21(cell + Vector2(0.0, 23.0 + float(i) * 13.0)) - 0.5) * _cell_size * _jitter
		)
		var pos := cell + jitter
...
func _place_instance(key: Vector3, cell: Vector2, pos: Vector2, type_idx: int, type_def: Dictionary, settings: Dictionary) -> bool:
	var yaw: float = MechanicNoise.hash21(cell + Vector2(99.0 + float(type_idx) * 7.0, 3.0)) * TAU
	var raw_roll: float = MechanicNoise.hash21(cell + Vector2(-7.0, 5.0 + float(type_idx) * 7.0))
	...
	var tilt: float = (MechanicNoise.hash21(cell + Vector2(41.0 + float(type_idx) * 7.0, -19.0)) - 0.5) * 2.0 * deg_to_rad(TILT_MAX_DEG)
	...
	MechanicAssetScan.set_visual_layers(proxy, MechanicAssetLayerScript.BLUR_BIT)
	MechanicAssetScan.set_flat_material(proxy, _blur_mask_material)
	MechanicAssetScan.set_instance_blur_param(proxy, float(settings["blur"]))
