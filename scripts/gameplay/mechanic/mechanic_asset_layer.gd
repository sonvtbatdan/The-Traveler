extends Node2D
class_name MechanicAssetLayer
## THE single 3D render pass for the Mechanic map's scattered assets — verbatim port of
## electric_asset_layer.gd (fully generic, no theme-specific logic; see that file's header for the full
## rationale on the two-pass color+blur-mask composite and the orthogonal-tilted camera math). No 3D
## cloud-occluder pass here — mirrors volcanic_asset_layer.gd's same omission (Mechanic's clouds are plain 2D
## parallax, not altitude-integrated with the scatter layer).

const COLOR_BIT := 1 << 0   # real scattered instances
const BLUR_BIT  := 1 << 1   # flat blur-id proxies (mechanic_blur_mask.gdshader)
...
const COMPOSITE_SHADER := "res://scripts/gameplay/mechanic/mechanic_asset_composite.gdshader"
...
## `world`: the shared World3D (mechanic_trees.gd.get_world_3d()).
