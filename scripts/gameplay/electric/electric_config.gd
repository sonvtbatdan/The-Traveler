extends RefCounted
class_name ElectricConfig
## Shared tunables for the Electric map's ground biome split. Both electric_ground.gdshader (GPU, paints the
## zone) and electric_trees.gd (CPU, decides which biome an asset scatter cell falls in) read the SAME
...
## electric_ground.gd/_make_material()).

const NOISE_FREQ := 0.0035        # world-space frequency fed into ElectricNoise.fbm()
...
## (river_width, exposed in the Terrain Edit panel / ElectricTerrainSettings) instead of a fixed one, so
## thicker width = more of the map counts as river. Both electric_ground.gdshader (GPU paint) and
## electric_trees.gd (CPU spawn-avoidance) read this same frequency/level so rivers and "no assets on rivers"
...
const RIVER_NOISE_FREQ := 0.001   # much lower than NOISE_FREQ -> a few big winding rivers, not many blotches
                                  # (matches electric_ground.gd's baked tex_river: RIVER_TILE_CYCLES/river tile size)
...
##                the whole Electric background, so this is structural, not a per-frame height check)
## electric_trees.gd's per-instance "above/below cloud" split compares each instance's effective rendered
