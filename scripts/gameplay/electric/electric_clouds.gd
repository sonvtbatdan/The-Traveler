extends Node2D
class_name ElectricClouds
## 3 soft procedural cloud layers drifting above the Electric ground — parallax via world_offset scaled by
## a per-layer factor < 1 (slower apparent motion = reads as farther away). Noise is ONE baked seamless
## NoiseTexture2D shared by all 3 layers (see electric_ground.gd for why baked > hand-rolled fbm here) —
## each layer gets visual variety from its own seed_offset/coverage/alpha, not a separate bake.
##
## World-space Node2D, NOT a CanvasLayer (unlike electric_ground.gd) — a CanvasLayer can only sit entirely
...
## instead of CanvasLayer. Ordering then falls out of normal add-order/z_index: see electric_trees.gd's header
## comment.
##
## Purely a decorative sky-atmosphere layer now — real occlusion of scattered assets against the clouds is
## handled by a separate, actual 3D cloud mesh living in electric_trees.gd's shared World3D (depth-tested
...
const CLOUD_SHADER := preload("res://scripts/gameplay/electric/electric_clouds.gdshader")
...
var _color: Color = ElectricTerrainSettings.DEFAULT_CLOUD_COLOR
var _clumpiness: float = ElectricTerrainSettings.DEFAULT_CLOUD_CLUMPINESS

func _ready() -> void:
	add_to_group("electric_clouds")   # so the Terrain Edit panel can find this instance
...
## gaps — see electric_clouds.gdshader's header.
