extends RefCounted
class_name ElectricTerrainSettings
## Persisted "Terrain Edit" tuning — res://electric_terrain.cfg, same static load()/save() shape as
## creep_info_panel.gd's overrides file. Loaded by electric_ground.gd / electric_clouds.gd / electric_trees.gd
## at their own _ready() (mirrors creep_info_panel's "each system applies overrides at its own _ready()"
## convention) and re-applied LIVE while the Terrain Edit panel (electric_terrain_edit.gd) is open.
##
## Density/Scale-range/Blur are PER-ASSET (asset_settings: type_name -> {density, scale_min, scale_max,
## blur}) — each discovered .glb (ElectricAssetScan) gets its own independent tuning, selected via the
...
const CFG_PATH := "res://electric_terrain.cfg"

const DEFAULT_ASSET_DENSITY := 50.0    # 0..100 — direct % chance per candidate cell this type spawns
                                        # independently of every other type (see electric_trees.gd's
...
const DEFAULT_CANOPY_SIZE := 1600.0     # world-px spanned by one canopy-photo tile — see electric_ground.gd
const DEFAULT_RIVER_BANK_COLOR := Color(0.87, 0.74, 0.40)   # golden sand fringe between canopy and water
const DEFAULT_WATER_COLOR := Color(0.15, 0.45, 0.65)         # base river water tone — see electric_ground.gd
const DEFAULT_WATER_WAVE_SIZE := 260.0  # world-px spanned by one wave-texture tile — see electric_ground.gd
const DEFAULT_CLOUD_CLUMPINESS := 0.65  # 0=soft misty veil, 1=distinct chunky masses w/ clear gaps — see
                                         # electric_clouds.gd/electric_trees.gd's cloud occluder
const DEFAULT_MAPTILE_SET := "green"    # subfolder of assets/map/electric/maptile/ — see electric_ground.gd
const DEFAULT_WATER_TILE_SET := "B"     # subfolder of assets/map/electric/watertile/ — see electric_ground.gd
const DEFAULT_LANDMARK_RING_WIDTH := 32.0   # world-px width of the worn-clearing band around each landmark
                                             # (e.g. the temple boss) — see electric_ground.gd/.gdshader
const DEFAULT_CANOPY_LIGHT_ANGLE_DEG := 55.0   # canopy normal-map lighting (electric_ground.gd's
...
const DEFAULT_CANOPY_LIGHT_COLOR := Color(1.0, 0.95, 0.85)   # "sun" tint — see electric_ground.gdshader's
                                                               # canopy_light_color
const DEFAULT_SPARK_AMOUNT := 40.0             # floating spark/dust-mote particle count (electric_sparks.gd) — 0 = off
