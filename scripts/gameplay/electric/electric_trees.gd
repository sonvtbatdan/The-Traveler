extends Node2D
class_name ElectricTrees
## Manages the Electric map's scattered 3D assets (trees/temples/etc, one per .glb in assets/map/electric/ —
## see ElectricAssetScan). Real, LIVE 3D models, not baked sprites — a static 2D bake captures one fixed
...
## OCCLUDER mesh (a translucent box at ElectricConfig.CLOUD_MIN_PX..CLOUD_MAX_PX, tracking the camera every
## frame), and runs the scatter/placement logic.
##
## Asset instances and the cloud occluder all render in ONE combined pass (ElectricAssetLayer) — this is
...
## (same mesh, flat electric_blur_mask.gdshader material, tagged with that instance's own blur amount via a
## per-instance shader uniform) rendered into a second pass. ElectricAssetLayer reads both passes and applies
## a per-pixel variable blur radius, so two asset types standing side by side can still have independently
## different blur amounts. See electric_asset_layer.gd's header for the full two-pass composite picture.
##
## Camera projection is ORTHOGONAL, tilted CAM_ISO_DEG (ElectricAssetLayer) — matches arena.gd's SHIP_ISO_DEG
...
## applied in ElectricAssetLayer.update_view and this script's _place_instance) cancels that out exactly.
##
## Altitude model: see ElectricConfig's TERRAIN_HEIGHT_PX/CLOUD_MIN_PX/CLOUD_MAX_PX/SHIP_HEIGHT_PX (single
## source of truth). CLOUD_ALTITUDE_PX below mirrors ElectricConfig.CLOUD_MAX_PX (the top of the cloud band) —
...
const ElectricNoise := preload("res://scripts/gameplay/electric/electric_noise.gd")
...
const ElectricAssetLayerScript := preload("res://scripts/gameplay/electric/electric_asset_layer.gd")
const CLOUD_OCCLUDER_SHADER := "res://scripts/gameplay/electric/electric_cloud_occluder.gdshader"
const BLUR_MASK_SHADER := "res://scripts/gameplay/electric/electric_blur_mask.gdshader"
...
const CLOUD_ALTITUDE_PX := ElectricConfig.CLOUD_MAX_PX   # informational only now — see header comment
...
var _asset_layer: ElectricAssetLayer
...
var _jitter: float = ElectricTerrainSettings.DEFAULT_JITTER   # fraction of _cell_size — Terrain Edit panel's Jitter slider
var _cell_size: float = CELL_SIZE_MAX   # recomputed each _regenerate() from the densest enabled type's density

func _ready() -> void:
	add_to_group("electric_trees")   # so the Terrain Edit panel can find this instance
	_z_comp = 1.0 / cos(deg_to_rad(ElectricAssetLayerScript.CAM_ISO_DEG))
	_build_host()
	_build_cloud_occluder()
	await get_tree().process_frame
	await _measure_types()

	_asset_layer = ElectricAssetLayerScript.new()
	...
	for t: Dictionary in _types:
		...
		_settings_by_type[t["name"]] = ElectricTerrainSettings.asset_entry(s, t["name"])
...
## Real 3D geometry, not a 2D trick — a single flat quad at the midpoint of ElectricConfig.CLOUD_MIN_PX..
...
func _build_cloud_occluder() -> void:
	...
	_cloud_occluder.layers = ElectricAssetLayerScript.COLOR_BIT
...
## (0..1) matches electric_clouds.gdshader's mass/detail treatment so the occluder's real depth-clipping shape
...
## (persisted settings). Keeps scattered assets off the SAME river band electric_ground.gdshader paints
## (ElectricNoise.is_river uses the same frequency/level) — width 0 disables river avoidance entirely.
...
func _measure_types() -> void:
	for glb_path: String in ElectricAssetScan.glb_paths():
...
## e.g. electric_temple_layer.gd, whose .glb is excluded from the regular pool — see
## ElectricAssetScan.SCATTER_EXCLUDED) — same AABB-probe measurement either way.
func _measure_one(glb_path: String) -> Dictionary:
	...
	var aabb := ElectricAssetScan.combined_aabb(probe)
	...
	probe.queue_free()
	return {
		"name": ElectricAssetScan.type_name(glb_path),
		"packed": packed,
...
## clear/rebuild. For one-off landmark/boss objects (e.g. the temple boss, electric_temple_layer.gd) whose
...
## model's actual BASE footprint (an oriented box, not a circle) for the ground ring (electric_ground.gd's
## apply_landmarks / electric_ground.gdshader) to hug the real base contact instead of floating a circle around
...
func spawn_landmark(glb_path: String, world_pos: Vector2, scale_mult: float = 1.0) -> Dictionary:
	...
	ElectricAssetScan.set_visual_layers(outer, ElectricAssetLayerScript.COLOR_BIT)
...
func update_view(center: Vector2, view_size: Vector2) -> void:
	...
	_cloud_occluder.position = Vector3(center.x, (ElectricConfig.CLOUD_MIN_PX + ElectricConfig.CLOUD_MAX_PX) * 0.5, center.y * _z_comp)
	if not _types_ready:
		return
...
func _regenerate(center: Vector2, force_full: bool = false) -> void:
	...
	for t: Dictionary in _types:
		var s: Dictionary = _settings_by_type.get(t["name"], ElectricTerrainSettings.default_asset_entry())
...
## Water/river avoidance (ElectricNoise.is_river) is the one remaining exclusion zone, checked per type against
...
func _maybe_place(cell: Vector2) -> void:
	if _types.is_empty():
		return
	var start_i: int = int(ElectricNoise.hash21(cell + Vector2(17.0, 17.0)) * _types.size())
	for k in _types.size():
		var i: int = (start_i + k) % _types.size()
		var type_def: Dictionary = _types[i]
		var settings: Dictionary = _settings_by_type.get(type_def["name"], ElectricTerrainSettings.default_asset_entry())
		...
		var roll: float = ElectricNoise.hash21(cell * 1.271 + Vector2(300.0 + float(i) * 47.0, 900.0 + float(i) * 91.0))
		if roll >= chance:
			continue
		if int(_count_by_type[i]) >= MAX_INSTANCES_PER_TYPE:
			continue
		var jitter := Vector2(
			(ElectricNoise.hash21(cell + Vector2(23.0 + float(i) * 13.0, 0.0)) - 0.5) * _cell_size * _jitter,
			(ElectricNoise.hash21(cell + Vector2(0.0, 23.0 + float(i) * 13.0)) - 0.5) * _cell_size * _jitter
		)
		var pos := cell + jitter
		if _river_width > 0.0 and ElectricNoise.is_river(pos, _river_width):
			continue
...
func _place_instance(key: Vector3, cell: Vector2, pos: Vector2, type_idx: int, type_def: Dictionary, settings: Dictionary) -> bool:
	var yaw: float = ElectricNoise.hash21(cell + Vector2(99.0 + float(type_idx) * 7.0, 3.0)) * TAU   # spin around the trunk only — stays upright
	var raw_roll: float = ElectricNoise.hash21(cell + Vector2(-7.0, 5.0 + float(type_idx) * 7.0))   # 0..1, this cell+type's fixed point in the range
	...
	# power < 1 (pushes most rolls toward 1 → mostly large). See ElectricTerrainSettings.DEFAULT_ASSET_SCALE_BIAS.
	...
	var tilt: float = (ElectricNoise.hash21(cell + Vector2(41.0 + float(type_idx) * 7.0, -19.0)) - 0.5) * 2.0 * deg_to_rad(TILT_MAX_DEG)
	...
	# only by the blur-mask pass's camera (ElectricAssetLayer.BLUR_BIT) — see this file's header comment.
	...
	ElectricAssetScan.set_visual_layers(proxy, ElectricAssetLayerScript.BLUR_BIT)
	ElectricAssetScan.set_flat_material(proxy, _blur_mask_material)
	ElectricAssetScan.set_instance_blur_param(proxy, float(settings["blur"]))
