extends RefCounted
class_name ElectricAssetScan
## Scans assets/map/electric/ for tree .glb models — the single source of truth both the offline bake tool
## (tools/bake_electric_trees.gd) and the runtime scatter (electric_trees.gd) read. Dropping a new .glb in
...
const FOLDER := "res://assets/map/electric/"
const SCATTER_EXCLUDED := ["temple", "constructor", "mechanic"]   # landmark/rescue objects spawned via a
                                        # dedicated system (electric_temple_layer.gd / electric_ruin_layer.gd),
...
const MAPTILE_FOLDER := FOLDER + "maptile/"     # one subfolder per ground tile SET (e.g. "green", "grey") —
                                                 ...
                                                 # electric_ground.gd's apply_maptile_set / Terrain Edit panel's
                                                 # "Tile Set" dropdown.
const WATERTILE_FOLDER := FOLDER + "watertile/" # one subfolder per water wave-texture SET (e.g. "B"/"C"/"D",
                                                 ...
                                                 # see electric_ground.gd's apply_water_tile_set / Terrain Edit
...
## Where tools/bake_electric_trees.gd writes (icon/reference use) the baked top-down PNG for a given
## .glb — same folder, same filename stem, .png extension. NOT used for in-game scattering any more
## (electric_trees.gd renders the live .glb directly so moving the camera actually reveals new angles,
...
## Display/lookup name for a .glb — its filename stem (e.g. "res://assets/map/electric/tree 1.glb" -> "tree
## 1"). Used as the key for per-asset settings (ElectricTerrainSettings.asset_sizes) and the Terrain Edit
...
## instance as "short" or "tall" (electric_trees.gd's cloud-altitude split: a Camera3D's cull_mask only
...
## material — used for the blur-mask proxy pass (electric_trees.gd/electric_asset_layer.gd), which needs the
