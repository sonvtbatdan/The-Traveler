extends RefCounted
class_name ElectricNoise
## CPU-side noise for Electric. biome_value() drives grass/tree placement and wraps Godot's own
...
## electric_ground.gdshader's history) that a from-scratch reimplementation risked repeating; FastNoiseLite
## is already proven in this codebase. The GPU ground/cloud shaders sample a NoiseTexture2D baked from a
## SEPARATE FastNoiseLite (see electric_ground.gd/_make_noise_tex) — the two aren't pixel-identical (the
...
static func _ensure() -> FastNoiseLite:
	if _fnl == null:
		...
		_fnl.frequency = ElectricConfig.NOISE_FREQ
		_fnl.seed = 1
	return _fnl

## 0..1 output (FastNoiseLite.get_noise_2d returns roughly -1..1) — same threshold convention the old
## hand-rolled fbm used, so ElectricConfig.BLUE_THRESHOLD/BLEND_SOFTNESS still apply unchanged.
...
static func _ensure_river() -> FastNoiseLite:
	if _fnl_river == null:
		...
		_fnl_river.frequency = ElectricConfig.RIVER_NOISE_FREQ
		_fnl_river.seed = 5   # different from biome's seed(1) so rivers don't line up with biome edges
	return _fnl_river

## 0..1 output for the river contour field — see ElectricConfig.RIVER_LEVEL/RIVER_NOISE_FREQ.
...
## electric_trees.gd to keep scattered assets off rivers.
static func is_river(world_pos: Vector2, half_width: float) -> bool:
	return absf(river_value(world_pos) - ElectricConfig.RIVER_LEVEL) < half_width
