extends Node2D
class_name ArcticAssetLayer
## THE single 3D render pass for the Arctic map's scattered assets — verbatim port of mechanic_asset_layer.gd
## (fully generic, no theme-specific logic; see that file's header for the full rationale on the two-pass
## color+blur-mask composite and the orthogonal-tilted camera math). No 3D cloud-occluder pass here — mirrors
## volcanic/mechanic_asset_layer.gd's same omission (Arctic's clouds are plain 2D parallax, not altitude-
## integrated with the scatter layer).

const COLOR_BIT := 1 << 0   # real scattered instances
const BLUR_BIT  := 1 << 1   # flat blur-id proxies (arctic_blur_mask.gdshader)
...
const COMPOSITE_SHADER := "res://scripts/gameplay/arctic/arctic_asset_composite.gdshader"
...
## `world`: the shared World3D (arctic_trees.gd.get_world_3d()).
