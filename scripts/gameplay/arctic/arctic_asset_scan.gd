extends RefCounted
class_name ArcticAssetScan
## Scans assets/map/arctic/ — port of mechanic_asset_scan.gd (see that file for the full rationale on scanning
## by .glb stem, not by .png, and on the one-subfolder-per-SET maptile/watertile convention). Dropping a new
## .glb directly in assets/map/arctic/ is the whole "add a scatter/landmark variant" workflow — no code changes.

const ArcticNoise := preload("res://scripts/gameplay/arctic/arctic_noise.gd")

const FOLDER := "res://assets/map/arctic/"
const SCATTER_EXCLUDED := []   # landmark model stems go here if one ever lives directly in FOLDER (mirrors
                                # electric's own ["temple","constructor"]) — Arctic's rescue-character glb
                                # (engineer.glb) lives in landmark/, a subfolder this scan never recurses into
                                # anyway, so nothing to exclude today.
const MAPTILE_FOLDER := FOLDER + "maptile/"     # one subfolder per ground tile SET — today just "default"
const WATERTILE_FOLDER := FOLDER + "watertile/" # one subfolder per water wave-texture SET — empty today

## Every .glb directly inside FOLDER (not recursive), sorted for stable ordering — excludes SCATTER_EXCLUDED
## (landmark models loaded directly by their own spawner instead).
...
## Every image file directly inside `folder`, sorted — shared by maptile_set_image_paths()/watertile_wave_path().
## `exclude_normal` skips "*_normal.png" companion files (see maptile_normal_path).
...
## matched. Excludes "*_normal.png" companion files. However many photos are dropped here IS the canopy blend
## count (see canopy_count() below and arctic_noise.gd's header) — unlike Mechanic's fixed 7, there's no
## required minimum beyond 1.
static func maptile_set_image_paths(set_name: String) -> Array:
	return _list_images(MAPTILE_FOLDER + set_name + "/", true)

## How many canopy photos the blend chain should actually use for `set_name`, clamped to ArcticNoise.
## MAX_CANOPY (the shader's fixed uniform-slot cap) — 0 if the set has no images at all (nothing to blend;
## caller should leave the ground unchanged rather than clear it out from under the shader, same "graceful
## empty" convention as apply_canopy_images' own size guard).
static func canopy_count(set_name: String) -> int:
	return mini(maptile_set_image_paths(set_name).size(), ArcticNoise.MAX_CANOPY)
...
## (ArcticTerrainSettings.asset_settings) and the Terrain Edit panel's per-asset sliders.
