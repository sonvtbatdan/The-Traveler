extends Node2D
class_name ArcticSparks
## Small glowing dust-mote sparks drifting through the Arctic map — port of mechanic_sparks.gd (see that
## file's header for the full rationale). A lightweight atmospheric flourish, independently tunable via the
## Light Edit panel (arctic_light_edit.gd's Spark sliders).
...
func _ready() -> void:
	add_to_group("arctic_sparks")   # so the Light Edit panel can find this instance
