extends CanvasLayer
class_name ArcticGround
## Screen-filling ground for the Arctic map — real top-down canopy PHOTOS (assets/map/arctic/maptile/default/),
## tinted per-zone and tiled at world-scale — ported from mechanic_ground.gd (see that file's header for the
## full baked-noise-texture / seamless-tiling rationale, identical here). Sits on a negative CanvasLayer
## (screen-space, always covers the viewport) — set_world_offset() shifts the CONTENT via a shader uniform
## each frame, same "rect stays put, uniform scrolls" trick as Electric/Mechanic/arena_nebula.
##
## ONE deliberate difference from MechanicGround (2026-08-19, on request — "hệ thống blend dynamic tự động
## phát hiện và blend dựa trên số ảnh có trong folder"): apply_canopy_images() below counts however many
## photos actually sit in the chosen maptile set (ArcticAssetScan.canopy_count(), currently 4) and pushes that
## as the shader's `canopy_count` uniform — Mechanic hardcodes 7 slots always fully used; Arctic's shader
## reserves up to ArcticNoise.MAX_CANOPY (10) slots but only the first `canopy_count` of them are ever
## selected (see arctic_ground.gdshader's header). Dropping a 5th+ photo into assets/map/arctic/maptile/
## default/ and reopening the map (or the Tile Set dropdown) picks it up automatically — no code change needed
## up to MAX_CANOPY.
##
## No landmark ring/patch system yet — no landmark .glb is scattered on this map (Arctic's rescue-character
## glb, engineer.glb, is placed by a future dedicated spawner, not this ground's ring system — see
## ArcticAssetScan's SCATTER_EXCLUDED note); apply_landmarks() can be ported back from electric_ground.gd once
## Arctic gets a scattered landmark.
##
## River: same seam-following + CORRIDOR-gate technique as Mechanic's final river design — see
## arctic_ground.gdshader's header and ArcticNoise.is_river() for the matching CPU-side avoidance formula.

const GROUND_SHADER := preload("res://scripts/gameplay/arctic/arctic_ground.gdshader")
...
const MOTTLE_TILE_CYCLES := 10.0   # baked-in noise detail for tex_mottle — the actual world-space "how far
                                     ...
                                     # a separate LIVE-tunable stretch on top of this fixed bake
const MOTTLE_OCTAVES := 2
const RIVER_TILE_CYCLES := 4.0   # baked-in cycles for tex_river's CORRIDOR gate — a low frequency so the
...
# tex_canopy_0..9 — matches arctic_ground.gdshader's 10 uniform names.
const CANOPY_SLOTS := 10

func _ready() -> void:
	add_to_group("arctic_ground")   # so the Terrain Edit panel can find this instance
	layer = -10
	_rect = ColorRect.new()
	_rect.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_mat = ShaderMaterial.new()
	_mat.shader = GROUND_SHADER
	_mat.set_shader_parameter("tex_zone", _make_noise_tex(ZONE_TILE_CYCLES, 4, 21))
	...
	_mat.set_shader_parameter("blue_threshold", ArcticConfig.BLUE_THRESHOLD)
	_mat.set_shader_parameter("blend_softness", ArcticConfig.BLEND_SOFTNESS)
	...
	apply_canopy_images(s["maptile_set"])   # sets _canopy_count BEFORE river_count is clamped against it
...
## _ready() (persisted settings). Clamped to `_canopy_count - 1` — there are never more live seams than that,
## however high the saved/UI value is. See arctic_ground.gdshader's header.
func apply_river_count(count: int) -> void:
	if _mat != null:
		_mat.set_shader_parameter("river_count", clampi(count, 0, maxi(0, _canopy_count - 1)))
...
## "Canopy Blend Sparseness" — world-px stretch on tex_mottle's baked pattern. Bigger = the region transitions
## land farther apart in world space, so more of any one photo shows intact between them.
func apply_canopy_mottle_scale(size_px: float) -> void:
	if _mat != null:
		_mat.set_shader_parameter("mottle_uv_scale", 1.0 / maxf(size_px, 1.0))

## Public: called by the Terrain Edit panel (live) and by this node's own _ready() (persisted settings).
## Smoothstep half-width (mottle-value space) around each region-threshold crossing — keep this NARROW; it's
...
## Public: called by the Terrain Edit panel's River tab (live, on slider change) and by this node's own
## _ready() (persisted settings). Extra noise-value half-width the icy sand fringe extends beyond river_width
## — 0 = canopy runs straight into the water with no fringe at all, bigger = a wider frosty bank.
...
## (persisted settings). Loads assets/map/arctic/watertile/<set_name>/water_wave.png as tex_water_wave. No-ops
## on "" (no watertile set exists yet) or if the set has no image, leaving whatever's already loaded.
func apply_water_tile_set(set_name: String) -> void:
	if _mat == null or set_name == "":
		return
	var path := ArcticAssetScan.watertile_wave_path(set_name)
	if path == "":
		push_warning("ArcticGround: watertile set '%s' has no image — water texture left unchanged." % set_name)
		return
	_mat.set_shader_parameter("tex_water_wave", load(path))

## Public: called by the Terrain Edit panel (live, on dropdown change) and by this node's own _ready()
## (persisted settings). Loads however many images sit in assets/map/arctic/maptile/<set_name>/ (positionally,
## up to ArcticNoise.MAX_CANOPY) as tex_canopy_0.. and pushes that count as the `canopy_count` uniform — see
## this file's header. No-ops (leaves whatever's already loaded) if the set has ZERO images — never clears
## textures out from under the shader.
func apply_canopy_images(set_name: String) -> void:
	if _mat == null:
		return
	var paths: Array = ArcticAssetScan.maptile_set_image_paths(set_name)
	if paths.is_empty():
		push_warning("ArcticGround: maptile set '%s' has no images — canopy textures left unchanged." % set_name)
		return
	_canopy_count = mini(paths.size(), ArcticNoise.MAX_CANOPY)
	if paths.size() > ArcticNoise.MAX_CANOPY:
		push_warning("ArcticGround: maptile set '%s' has %d images, only the first %d are used (ArcticNoise.MAX_CANOPY) — raise MAX_CANOPY to use more." % [set_name, paths.size(), ArcticNoise.MAX_CANOPY])
	_mat.set_shader_parameter("canopy_count", _canopy_count)
	for i in _canopy_count:
		_mat.set_shader_parameter("tex_canopy_%d" % i, load(paths[i]))
		_mat.set_shader_parameter("tex_canopy_%d_normal" % i, _load_normal_or_flat(paths[i]))

## Current auto-detected canopy count — read by arctic_trees.gd/arctic_plumes.gd (via ArcticAssetScan.
## canopy_count() directly, not this getter, so they don't need a live reference to this node) and by
## arctic_terrain_edit.gd to cap the River Count slider's range.
func canopy_count() -> int:
	return _canopy_count

## tools/generate_canopy_normal.py may not have been run yet for a given image — fall back to a flat neutral
## (0,0,1 -> RGB 128,128,255, "no relief") 1x1 texture. Mirrors MechanicGround's own fallback.
func _load_normal_or_flat(color_path: String) -> Texture2D:
	var normal_path := ArcticAssetScan.maptile_normal_path(color_path)
