extends Node2D
class_name ArcticTrees
## Manages the Arctic map's scattered 3D assets (one per .glb in assets/map/arctic/ — see ArcticAssetScan).
## Port of mechanic_trees.gd's fully generic scatter/compositor system (see that file's header for the
## complete rationale — shared World3D "host" viewport, two-pass color+blur-mask render, orthogonal tilted
## camera), INCLUDING spawn_landmark() (for a future rescue-character landmark spawner — Engineer is already
## assigned to Arctic, see [[traveler_rubicon_ruin_landmarks]], but no arctic_ruin_layer.gd exists yet; this
## just makes spawn_landmark() available for whenever one is built). No real 3D cloud-occluder plane —
## Arctic's clouds are plain 2D parallax (arctic_clouds.gd), matching Mechanic's own omission.
##
## assets/map/arctic/ currently has NO .glb files directly inside it (only enemy sprite PNGs in enemies/ and
## the maptile canopy photos in maptile/ — both subfolders this scan doesn't recurse into, plus the rescue-
## character glb in landmark/, also a subfolder), so _types stays empty and the regular density-scatter places
## nothing — same "drop a .glb, no code changes" convention Electric/Volcanic/Mechanic already document.
...
const ArcticAssetLayerScript := preload("res://scripts/gameplay/arctic/arctic_asset_layer.gd")
const BLUR_MASK_SHADER := "res://scripts/gameplay/arctic/arctic_blur_mask.gdshader"
...
const MARGIN := 140.0
const CLOUD_ALTITUDE_PX := ArcticConfig.CLOUD_MAX_PX   # informational only (Terrain Edit panel's Height
                                                         # readout) — no occluder mesh clips against this
const TILT_MAX_DEG := 8.0

var _host_vp: SubViewport
var _world3d_root: Node3D
var _asset_layer: ArcticAssetLayer
...
var _canopy_count: int = 4   # refreshed via apply_maptile_set() — see this file's own header on why it's tracked here too
var _placed_positions: Array = []
var _jitter: float = ArcticTerrainSettings.DEFAULT_JITTER
var _cell_size: float = CELL_SIZE_MAX

func _ready() -> void:
	add_to_group("arctic_trees")   # so the Terrain Edit panel can find this instance
	_z_comp = 1.0 / cos(deg_to_rad(ArcticAssetLayerScript.CAM_ISO_DEG))
	_build_host()
	await get_tree().process_frame
	await _measure_types()

	_asset_layer = ArcticAssetLayerScript.new()
	...
	for t: Dictionary in _types:
		...
		_settings_by_type[t["name"]] = ArcticTerrainSettings.asset_entry(s, t["name"])
...
## Public: keeps scattered assets off the SAME river band arctic_ground.gdshader paints (ArcticNoise.
## is_river() mirrors the shader's seam-based river_dist exactly) — width 0 disables avoidance entirely.
...
## node's own _ready() (persisted settings) — ArcticNoise.is_river()'s seam-based river check depends on the
## CURRENT mottle scale, same as arctic_plumes.gd's own tracking of it for ground_region().
func apply_canopy_mottle_scale(size_px: float) -> void:
	_mottle_uv_scale = 1.0 / maxf(size_px, 1.0)
	if _types_ready:
		_regenerate(_last_center, true)

## Public: called by the Terrain Edit panel (live, on "River Count" slider change) and by this node's own
## _ready() (persisted settings) — how many of the live canopy-blend seams carry a river, see ArcticNoise.
## is_river()'s header.
...
## _ready() (persisted settings) — refreshes _canopy_count from however many photos the chosen set actually
## has (ArcticAssetScan.canopy_count()), since ArcticNoise.is_river()'s seam-based check depends on it.
func apply_maptile_set(set_name: String) -> void:
	_canopy_count = ArcticAssetScan.canopy_count(set_name)
	if _types_ready:
		_regenerate(_last_center, true)
...
func _measure_types() -> void:
	for glb_path: String in ArcticAssetScan.glb_paths():
...
func _measure_one(glb_path: String) -> Dictionary:
	...
	await get_tree().process_frame
	var aabb := ArcticAssetScan.combined_aabb(probe)
	...
	probe.queue_free()
	return {
		"name": ArcticAssetScan.type_name(glb_path),
		"packed": packed,
...
## system entirely — for a future rescue-character landmark spawner (Engineer, arctic_ruin_layer.gd, not
## built yet). Verbatim port of mechanic_trees.gd's spawn_landmark() — see that file for the full rationale.
func spawn_landmark(glb_path: String, world_pos: Vector2, scale_mult: float = 1.0) -> Dictionary:
	...
	ArcticAssetScan.set_visual_layers(outer, ArcticAssetLayerScript.COLOR_BIT)
...
## settings, via _settings_by_type directly).
...
func _regenerate(center: Vector2, force_full: bool = false) -> void:
	...
	for t: Dictionary in _types:
		var s: Dictionary = _settings_by_type.get(t["name"], ArcticTerrainSettings.default_asset_entry())
...
func _maybe_place(cell: Vector2) -> void:
	if _types.is_empty():
		return
	var start_i: int = int(ArcticNoise.hash21(cell + Vector2(17.0, 17.0)) * _types.size())
	for k in _types.size():
		var i: int = (start_i + k) % _types.size()
		var type_def: Dictionary = _types[i]
		var settings: Dictionary = _settings_by_type.get(type_def["name"], ArcticTerrainSettings.default_asset_entry())
		...
		var roll: float = ArcticNoise.hash21(cell * 1.271 + Vector2(300.0 + float(i) * 47.0, 900.0 + float(i) * 91.0))
		if roll >= chance:
			continue
		if int(_count_by_type[i]) >= MAX_INSTANCES_PER_TYPE:
			continue
		var jitter := Vector2(
			(ArcticNoise.hash21(cell + Vector2(23.0 + float(i) * 13.0, 0.0)) - 0.5) * _cell_size * _jitter,
			(ArcticNoise.hash21(cell + Vector2(0.0, 23.0 + float(i) * 13.0)) - 0.5) * _cell_size * _jitter
		)
		var pos := cell + jitter
...
func _place_instance(key: Vector3, cell: Vector2, pos: Vector2, type_idx: int, type_def: Dictionary, settings: Dictionary) -> bool:
	var yaw: float = ArcticNoise.hash21(cell + Vector2(99.0 + float(type_idx) * 7.0, 3.0)) * TAU
	var raw_roll: float = ArcticNoise.hash21(cell + Vector2(-7.0, 5.0 + float(type_idx) * 7.0))
	...
	var tilt: float = (ArcticNoise.hash21(cell + Vector2(41.0 + float(type_idx) * 7.0, -19.0)) - 0.5) * 2.0 * deg_to_rad(TILT_MAX_DEG)
	...
	ArcticAssetScan.set_visual_layers(proxy, ArcticAssetLayerScript.BLUR_BIT)
	ArcticAssetScan.set_flat_material(proxy, _blur_mask_material)
	ArcticAssetScan.set_instance_blur_param(proxy, float(settings["blur"]))
