extends Control
## Standalone viewer for the Nebular Serpent model.
##
## Renders the model in a 3D SubViewport, brightened via emission sampled from its own texture
## (same colours, just brighter — ramped up toward the tail). Auto-spins so you can watch it;
## drag / keys to rotate manually, Space to play/pause its baked animation.
##
## Controls:  drag = orbit · ←→/AD yaw · ↑↓/WS pitch · Q/E roll · R reset · Space anim · Z toggle auto-spin
##
## Brightness knobs: BODY_GAIN (whole body), TAIL_GAIN (extra at the tail), TAIL_SIGN (flip if the
## HEAD glows instead of the tail).

const MODEL_PATH := "res://assets/3D models/Nebular Serpent.glb"

# ── Brightness ──
const BODY_GAIN := 0.35    # whole-body emission boost
const TAIL_GAIN := 1.20    # EXTRA emission ramped toward the tail (on top of BODY_GAIN)
const TAIL_SIGN := 1.0     # +1 = tail is the max end of the long axis, -1 = min end (flip if wrong)
...
# ── Rotation ──
const AUTO_SPIN_DPS := 20.0   # idle yaw, deg/s (Z toggles)
...
func _ready() -> void:
	...
	if model != null:
		...
		push_warning("nebula_snake_test: could not load model at " + MODEL_PATH)
...
func _update_label() -> void:
	...
	_label.text = "NEBULAR SERPENT   ·   auto-spin %s\ndrag orbit · ←→/AD ↑↓/WS Q/E rotate · R reset · Z spin%s" % [spin, anim_hint]

## Frame the camera to fit the model and recenter it so it rotates about its own middle.
...
## Brighten via emission sampled from the model's own texture, ramped toward the tail.
func _apply_brightness(model: Node3D) -> void:
	for mi: MeshInstance3D in _all_mesh_instances(model):
		...
		var box := mi.get_aabb()
		var s := box.size
		var axis := Vector3(1, 0, 0)                    # longest axis ≈ head→tail
...
## Play (and loop) the model's first baked animation, if any.
