...
func setup(oc: Control) -> void:
	...
	_load_fp_offsets()
	_load_assets()
	# Boss-killed routing is handled by the Boss Manager (boss_fight.gd), which is the
...
func _force_reset() -> void:
	...
	# weapons read), and force ONE more _draw() with phase = IDLE so the M3 warning
	# overlay doesn't freeze on the canvas after queue_redraw() stops next frame.
...
func kill_boss() -> void:
	...
	_force_reset()        # single source of truth: cleanup, hide, zero HP, clear overlay
...
func _process(delta: float) -> void:
	...
	# no boss HP) so the sprite can never linger on the asteroid screen.
	if _boss_eo != null and is_instance_valid(_boss_eo):
		if _phase == Phase.IDLE or _phase == Phase.DONE or GameManager.boss_max_hp <= 0:
			_boss_eo.visible = false
