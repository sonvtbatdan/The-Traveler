...
func _fire_red_x() -> void:
	...
	var ship_rot := _player.rotation          # radians — X arms rotate with the ship
	...
	for en in get_tree().get_nodes_in_group("arena_enemy"):
		...
		if dist < RED_X_INNER:
			continue
		var a := fposmod(off.angle() - ship_rot, PI / 2.0)  # fold into 4-fold in ship-local space
		var d_to_diag := absf(a - PI / 4.0)
	...
	_spawn_red_x_fire(center, RED_X_REACH, ship_rot)
