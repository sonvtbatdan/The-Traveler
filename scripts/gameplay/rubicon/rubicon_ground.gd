...
## Screen-filling ground for the Rubicon map — real top-down jungle-canopy PHOTOS (assets/map/rubicon/maptile/
## <set>/), tinted per-zone and tiled at world-scale, sitting under the scattered real 3D trees (RubiconTrees).
## Which SET of 3 photos is active (RubiconAssetScan.maptile_set_names(), e.g. "green"/"grey") is picked via
...
## see rubicon_ground.gdshader's header for how that's made acceptable (same precedent already used for the
...
## assets/map/rubicon/watertile/<B|C|D>/water_wave.png are copies of SeaWaterMaterial's Sea_Water_B/C/D
...
# slider, RubiconTerrainSettings.DEFAULT_CANOPY_SIZE=1600) controls how big individual leaf clusters read
# relative to the scattered 3D trees on top of them (RubiconTrees.DESIRED_HEIGHT_PX = 110) — too small and it
...
const LANDMARK_PATCH_RADIUS := 320.0     # world-px the guaranteed-dry ground patch extends beyond a landmark's
                                          # own ring (landmark_ring_width) — see rubicon_ground.gdshader
...
const MAX_LANDMARKS := 8            # must match rubicon_ground.gdshader's landmark_pos/landmark_half_extent/
...
## over the canopy photo textures (see rubicon_ground.gdshader's canopy_tint_strength), not a flat fill.
...
## rubicon_ground.gdshader's bank_mask.
...
## picker controls the whole wave-crest gradient (see rubicon_ground.gdshader's wave_mix blend).
...
## (persisted settings). Loads assets/map/rubicon/watertile/<set_name>/water_wave.png as tex_water_wave. Falls
## back to leaving whatever's already loaded if the set has no image yet.
func apply_water_tile_set(set_name: String) -> void:
	if _mat == null:
		return
	var path := RubiconAssetScan.watertile_wave_path(set_name)
	if path == "":
		push_warning("RubiconGround: watertile set '%s' has no image — water texture left unchanged." % set_name)
		return
...
## see RubiconAssetScan.maptile_set_image_paths, sets aren't required to share a filename convention) as
...
func apply_maptile_set(set_name: String) -> void:
	if _mat == null:
		return
	var paths: Array = RubiconAssetScan.maptile_set_image_paths(set_name)
	if paths.size() < 3:
		push_warning("RubiconGround: maptile set '%s' has %d image(s), need at least 3 — canopy textures left unchanged." % [set_name, paths.size()])
		return
...
func _load_normal_or_flat(color_path: String) -> Texture2D:
	var normal_path := RubiconAssetScan.maptile_normal_path(color_path)
