extends Node2D
## Spawner for the Electric (rubicon) map's rescue-character ruin landmark — constructor.glb/mechanic.glb
## (assets/map/rubicon/, moved there 2026-08-06 from assets/ruin/) rendered/fought exactly like
## rubicon_temple_layer.gd's temple boss (RubiconTrees.spawn_landmark() for the live .glb, arena_enemy.gd as
## the 2D combat/hit-detection vehicle, arena_ruin_pointer.gd for the edge-of-screen arrow, RubiconGround.
## apply_landmarks for the dry-land clearing ring — see rubicon_temple_layer.gd's own header for the full
## rundown of that shared plumbing), but:
##   - AT MOST ONE spawns per run (not always all of them) — MetaManager.rescue_candidate_for_map("rubicon")
##     decides WHICH character (constructor until rescued, then mechanic, then Scholar once every map's own
##     pair is rescued — see its own doc comment) is this run's target, or "" if nothing is left to offer.
...
const DIST_MIN := 10000.0   # minimum spawn distance from player (px) — matches rubicon_temple_layer.gd's own
...
const ENEMY_HP_TUNE := 2.0   # arena_enemy.gd's global ×2 HP tune for every non-"boss_stub" enemy — divide it
                              ...
                              # compensation rubicon_temple_layer.gd's own TEMPLE_HP does)
...
func _ready() -> void:
	...
	if _player == null or not is_instance_valid(_player):
		...
		return
	var char_id := MetaManager.rescue_candidate_for_map("rubicon")
	if char_id == "":
		return   # both constructor+mechanic already rescued, and Scholar not eligible here yet (or done too)
	var angle := randf() * TAU
...
## ground's ring set. On a GENUINE kill (gated on the enemy's own `_dead` flag, same convention rubicon_
...
func _push_landmarks() -> void:
	...
	ground.call("apply_landmarks", landmarks, "ruin")   # own source key — see RubiconGround.apply_landmarks

## Edge-of-screen arrow + live distance guiding the player to the ruin (mirrors rubicon_temple_layer.gd's own
