extends Node2D
class_name RubiconSparks
## Small glowing dust-mote/firefly sparks drifting through the Rubicon jungle — a lightweight atmospheric
## flourish, independently tunable via the Light Edit panel (rubicon_light_edit.gd's Spark Amount/Speed/Size/
## Direction/Color/Brightness sliders). Not tied to any actual light source (this map's 2D layers stay unlit —
## see rubicon_ground.gdshader's header).
##
## World-space Node2D (not a CanvasLayer), same "tracks the camera via its own global_position every frame"
## convention as rubicon_clouds.gd. Uses a single CPUParticles2D with local_coords=false so already-emitted
...
func _ready() -> void:
	add_to_group("rubicon_sparks")   # so the Light Edit panel can find this instance
...
## Call every frame with the camera's world-space focus — mirrors rubicon_clouds.gd's set_world_offset.
