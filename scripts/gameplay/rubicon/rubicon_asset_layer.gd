extends Node2D
class_name RubiconAssetLayer
## THE single 3D render pass for the Rubicon map's scattered assets — rubicon_trees.gd adds every scattered
...
## instance (RubiconAssetScan.set_flat_material + set_instance_blur_param, built by rubicon_trees.gd) whose
## pixels encode that instance's own blur amount. rubicon_asset_composite.gdshader reads both textures and
...
const BLUR_BIT  := 1 << 1   # flat blur-id proxies (rubicon_blur_mask.gdshader)
...
const COMPOSITE_SHADER := "res://scripts/gameplay/rubicon/rubicon_asset_composite.gdshader"
...
## `world`: the shared World3D (rubicon_trees.gd.get_world_3d()).
...
func update_view(center: Vector2, view_size: Vector2) -> void:
	...
	if _color_cam == null:
		return
	global_position = center   # composite rect stays screen-covering, same trick as rubicon_clouds.gd
