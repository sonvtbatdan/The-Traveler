...
## a specific rendered reference image, not portable from Volcanic's own (different model) reference.
##
...
const SPAWN_WINDOW := 1200.0  # 20 minutes, in seconds — matches Electric/Volcanic's own window
...
## Must match tools/bake_atlantic_landmark.gd's own copy — the world-unit span (both axes, square frame) the
...
func _ready() -> void:
	add_to_group("atlantic_temple_layer")   # so atlantic_landmark_mark.gd can find this instance and re-sync
...
## value"). Guard it like any other post-teardown callback.
func _on_temple_gone(entry: Dictionary) -> void:
	...
	var tree := get_tree()
	if tree != null:
		var clouds := tree.get_first_node_in_group("atlantic_clouds")
...
## Public: called by atlantic_landmark_mark.gd (live, whenever a mark is added/removed on either tab) — re-
...
func _push_plumes_for(entry: Dictionary) -> void:
	...
	var s := AtlanticTerrainSettings.load_settings()
	var entries: Array = []
	for kind: String in ["bubble", "whirlpool"]:
