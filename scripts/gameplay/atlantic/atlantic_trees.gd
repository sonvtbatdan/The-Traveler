extends Node2D
class_name AtlanticTrees
## Manages the Atlantic map's scattered 3D assets (coral/wreckage debris, one per .glb in
...
const AtlanticAssetLayerScript := preload("res://scripts/gameplay/atlantic/atlantic_asset_layer.gd")
const BLUR_MASK_SHADER := "res://scripts/gameplay/atlantic/atlantic_blur_mask.gdshader"
...
const MARGIN := 140.0
const CLOUD_ALTITUDE_PX := AtlanticConfig.CLOUD_MAX_PX   # informational only (Terrain Edit panel's Height readout)
const TILT_MAX_DEG := 8.0

var _host_vp: SubViewport
var _world3d_root: Node3D
var _asset_layer: AtlanticAssetLayer
...
var _jitter: float = AtlanticTerrainSettings.DEFAULT_JITTER
var _cell_size: float = CELL_SIZE_MAX

func _ready() -> void:
	add_to_group("atlantic_trees")   # so the Terrain Edit panel can find this instance
	_z_comp = 1.0 / cos(deg_to_rad(AtlanticAssetLayerScript.CAM_ISO_DEG))
	_build_host()
	await get_tree().process_frame
	await _measure_types()

	_asset_layer = AtlanticAssetLayerScript.new()
	...
	for t: Dictionary in _types:
		...
		_settings_by_type[t["name"]] = AtlanticTerrainSettings.asset_entry(s, t["name"])
...
func _build_host() -> void:
	...
	env.ambient_light_color = Color(0.75, 0.90, 0.98)   # cool blue-tinted ambient, underwater caustic feel
...
## Public: the shared World3D "host" root — any Node3D added here is automatically picked up by
## AtlanticAssetLayer's existing color-pass camera/compositor (no separate viewport/camera needed), same as
## every scattered coral/wreckage instance and atlantic_temple_layer.gd's landmark. Added 2026-08-08 so
## atlantic_clouds.gd can spawn a real 3D whirlpool-funnel mesh instead of a flat 2D disc (on request: "tôi muốn
## xoay vfx này để có thể nhìn thấy thân Whirlpool"). Null until _build_host() runs (start of THIS node's own
## _ready(), before any awaits — safe for a sibling's _ready() to call immediately).
func get_world3d_root() -> Node3D:
	return _world3d_root

## Public: world-px-Y -> 3D-Z scale factor (see spawn_landmark()'s own Vector3(pos.x, 0.0, pos.y * _z_comp)) —
## callers positioning their own Node3D in this shared world need the same conversion.
func get_z_comp() -> float:
	return _z_comp

## Public: keeps scattered assets off the SAME current band atlantic_ground.gdshader paints (AtlanticNoise.
## is_river uses the same frequency/level) — width 0 disables avoidance entirely.
...
func _measure_types() -> void:
	for glb_path: String in AtlanticAssetScan.glb_paths():
...
func _measure_one(glb_path: String) -> Dictionary:
	...
	await get_tree().process_frame
	var aabb := AtlanticAssetScan.combined_aabb(probe)
	...
	probe.queue_free()
	return {
		"name": AtlanticAssetScan.type_name(glb_path),
		"packed": packed,
...
func spawn_landmark(glb_path: String, world_pos: Vector2, scale_mult: float = 1.0) -> Dictionary:
	...
	AtlanticAssetScan.set_visual_layers(outer, AtlanticAssetLayerScript.COLOR_BIT)
...
## settings, via _settings_by_type directly). See volcanic_trees.gd's apply_asset_setting for full semantics.
...
func _regenerate(center: Vector2, force_full: bool = false) -> void:
	...
	for t: Dictionary in _types:
		var s: Dictionary = _settings_by_type.get(t["name"], AtlanticTerrainSettings.default_asset_entry())
...
func _maybe_place(cell: Vector2) -> void:
	if _types.is_empty():
		return
	var start_i: int = int(AtlanticNoise.hash21(cell + Vector2(17.0, 17.0)) * _types.size())
	for k in _types.size():
		var i: int = (start_i + k) % _types.size()
		var type_def: Dictionary = _types[i]
		var settings: Dictionary = _settings_by_type.get(type_def["name"], AtlanticTerrainSettings.default_asset_entry())
		...
		var roll: float = AtlanticNoise.hash21(cell * 1.271 + Vector2(300.0 + float(i) * 47.0, 900.0 + float(i) * 91.0))
		if roll >= chance:
			continue
		if int(_count_by_type[i]) >= MAX_INSTANCES_PER_TYPE:
			continue
		var jitter := Vector2(
			(AtlanticNoise.hash21(cell + Vector2(23.0 + float(i) * 13.0, 0.0)) - 0.5) * _cell_size * _jitter,
			(AtlanticNoise.hash21(cell + Vector2(0.0, 23.0 + float(i) * 13.0)) - 0.5) * _cell_size * _jitter
		)
		var pos := cell + jitter
...
func _place_instance(key: Vector3, cell: Vector2, pos: Vector2, type_idx: int, type_def: Dictionary, settings: Dictionary) -> bool:
	var yaw: float = AtlanticNoise.hash21(cell + Vector2(99.0 + float(type_idx) * 7.0, 3.0)) * TAU
	var raw_roll: float = AtlanticNoise.hash21(cell + Vector2(-7.0, 5.0 + float(type_idx) * 7.0))
	...
	var tilt: float = (AtlanticNoise.hash21(cell + Vector2(41.0 + float(type_idx) * 7.0, -19.0)) - 0.5) * 2.0 * deg_to_rad(TILT_MAX_DEG)
	...
	AtlanticAssetScan.set_visual_layers(proxy, AtlanticAssetLayerScript.BLUR_BIT)
	AtlanticAssetScan.set_flat_material(proxy, _blur_mask_material)
	AtlanticAssetScan.set_instance_blur_param(proxy, float(settings["blur"]))
