extends RefCounted
class_name AtlanticAssetScan
## Scans assets/map/atlantic/ for coral/wreckage .glb models — mirrors volcanic/volcanic_asset_scan.gd exactly
## (see that file for the full rationale on scanning by .glb stem, not by .png). Dropping a new .glb in this
## folder is the whole "add a coral/wreckage variant" workflow — no code edits. Empty on launch (no scatter
...
const MAPTILE_FOLDER := FOLDER + "maptile/"     # one subfolder per ground tile SET — see atlantic_ground.gd's
                                                 # apply_maptile_set / Terrain Edit panel's "Tile Set" dropdown.
const WATERTILE_FOLDER := FOLDER + "watertile/" # one subfolder per current wave-texture SET — see
                                                 # atlantic_ground.gd's apply_water_tile_set / Terrain Edit
                                                 # panel's "Current Pattern" dropdown.
...
## Every subfolder directly inside WATERTILE_FOLDER, sorted — one entry per available current wave-texture set.
...
## stem, .png extension. Mirrors VolcanicAssetScan.baked_png_path.
...
## (AtlanticTerrainSettings.asset_sizes) and the Terrain Edit panel's per-asset Size sliders.
static func type_name(glb_path: String) -> String:
	return glb_path.get_file().get_basename()

## Recursively sets `.layers` on every VisualInstance3D under `root` — see VolcanicAssetScan.set_visual_layers.
...
## VolcanicAssetScan.set_flat_material.
...
## VolcanicAssetScan.set_instance_blur_param.
...
## VolcanicAssetScan.combined_aabb (same "must already be inside the SceneTree for a frame" caveat applies).
