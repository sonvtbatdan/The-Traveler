extends Node2D
class_name AtlanticAssetLayer
## THE single 3D render pass for the Atlantic map's scattered assets — verbatim port of
## volcanic/volcanic_asset_layer.gd (fully generic, no theme-specific logic; see that file's header for the
...
const BLUR_BIT  := 1 << 1   # flat blur-id proxies (atlantic_blur_mask.gdshader)
...
const COMPOSITE_SHADER := "res://scripts/gameplay/atlantic/atlantic_asset_composite.gdshader"
...
## `world`: the shared World3D (atlantic_trees.gd.get_world_3d()).
