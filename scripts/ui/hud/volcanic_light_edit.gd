...
func _ready() -> void:
	layer = 61   # same tier as volcanic_terrain_edit.gd / creep_info_panel.gd
	process_mode = Node.PROCESS_MODE_ALWAYS
	add_to_group("volcanic_light_edit")
...
func _build_ui() -> void:
	...
	sb.border_color = Color(1.0, 0.55, 0.15)   # hot ember border — visually distinct from Terrain Edit's
	...
	_font(title, 15, Color(1.0, 0.85, 0.6))
	...
	_add_color_row(col, "ground_light_color", "Light Color (sun)")
	col.add_child(HSeparator.new())
	_add_slider(col, "spark_amount", "Ember Amount", 0.0, 150.0, 5.0)
	_add_slider(col, "spark_speed", "Ember Speed (px/s)", 0.0, 100.0, 1.0)
	_add_slider(col, "spark_size", "Ember Size (px)", 0.0, 100.0, 1.0)
	_add_slider(col, "spark_direction_deg", "Ember Direction", 0.0, 360.0, 5.0)
	_add_slider(col, "spark_brightness", "Ember Brightness", 0.0, 3.0, 0.05)
	_add_slider(col, "spark_opacity", "Ember Opacity", 0.0, 1.0, 0.02)
	_add_color_row(col, "spark_color", "Ember Color")
...
func _add_slider(parent: VBoxContainer, key: String, label_text: String, min_v: float, max_v: float, step_v: float) -> void:
	var lbl := Label.new()
	lbl.text = label_text
	_font(lbl, 13, Color(0.9, 0.85, 0.8))
	...
	_font(val_lbl, 13, Color(1.0, 0.7, 0.3))
...
func _apply_live() -> void:
	...
	var sparks := get_tree().get_first_node_in_group("volcanic_sparks")
...
func _on_reset() -> void:
	_values["ground_light_angle_deg"] = VolcanicTerrainSettings.DEFAULT_GROUND_LIGHT_ANGLE_DEG
	_values["ground_light_height"] = VolcanicTerrainSettings.DEFAULT_GROUND_LIGHT_HEIGHT
	_values["ground_ambient"] = VolcanicTerrainSettings.DEFAULT_GROUND_AMBIENT
	_values["ground_contrast"] = VolcanicTerrainSettings.DEFAULT_GROUND_CONTRAST
	_values["ground_specular"] = VolcanicTerrainSettings.DEFAULT_GROUND_SPECULAR
	_values["ground_light_color"] = VolcanicTerrainSettings.DEFAULT_GROUND_LIGHT_COLOR
	_values["spark_amount"] = VolcanicTerrainSettings.DEFAULT_SPARK_AMOUNT
	_values["spark_color"] = VolcanicTerrainSettings.DEFAULT_SPARK_COLOR
	_values["spark_speed"] = VolcanicTerrainSettings.DEFAULT_SPARK_SPEED
	_values["spark_size"] = VolcanicTerrainSettings.DEFAULT_SPARK_SIZE
	_values["spark_direction_deg"] = VolcanicTerrainSettings.DEFAULT_SPARK_DIRECTION_DEG
	_values["spark_brightness"] = VolcanicTerrainSettings.DEFAULT_SPARK_BRIGHTNESS
	_values["spark_opacity"] = VolcanicTerrainSettings.DEFAULT_SPARK_OPACITY
	_sync_ui_from_values()
	_apply_live()
	if _status != null:
