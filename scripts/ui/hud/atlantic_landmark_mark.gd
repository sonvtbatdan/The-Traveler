extends CanvasLayer
class_name AtlanticLandmarkMark
...
## re-derives each spawned instance's actual world attach points from these same local marks every time it
## (re)spawns or this panel edits live.
##
## TWO TABS — "BUBBLE" / "WHIRLPOOL" (same split as atlantic_crater_mark.gd) — independent mark lists
## (`landmark_marks_bubble`/`landmark_marks_whirlpool`), only the active tab's dots show.
##
## Left-click = add a mark for the active tab. Right-click near an existing mark (active tab) = remove it.
## Applies LIVE by calling atlantic_temple_layer.gd's refresh_landmark_marks() (re-registers plumes on every
## CURRENTLY SPAWNED landmark instance — a mark added here does nothing until at least one temple exists, same
## as every other Atlantic mark tool needing its target system already running). SAVE persists to
## res://atlantic_terrain.cfg via AtlanticTerrainSettings — every add/remove auto-saves immediately, scoped to
## ONLY the landmark_marks_* keys so it never clobbers other panels' unsaved edits.
...
const REMOVE_CLICK_RADIUS := 0.05   # normalized frame-fraction distance — right-click must land within this
                                     # of a mark to remove it (same scale as atlantic_crater_mark.gd's UV space)
...
var _values: Dictionary = {}   # full settings dict — we mutate _values["landmark_marks_bubble"/"_whirlpool"]
...
func _ready() -> void:
	...
	add_to_group("atlantic_landmark_mark")
...
func _build_ui() -> void:
	...
	_font(title, 15, Color(0.8, 0.95, 0.9))
	...
	_font(hint, 10, Color(0.55, 0.65, 0.65))
	...
	if not ResourceLoader.exists(REF_IMAGE_PATH):
		...
		_missing_lbl.text = "No reference image yet.\nRun: godot --path . --script tools/bake_atlantic_landmark.gd"
		_font(_missing_lbl, 13, Color(1.0, 0.7, 0.6))
		...
		checker.color = Color(0.08, 0.14, 0.17)   # deep-water backdrop instead of Volcanic's neutral grey
	...
	_font(_count_lbl, 12, Color(0.8, 0.88, 0.85))
...
func _push_live() -> void:
	var layer := get_tree().get_first_node_in_group("atlantic_temple_layer")
	if layer != null and layer.has_method("refresh_landmark_marks"):
		layer.call("refresh_landmark_marks")

## Marking is a discrete, deliberate click — same reasoning/fix as atlantic_crater_mark.gd's _commit(): every
