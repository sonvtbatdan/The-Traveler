extends CanvasLayer
class_name ElectricLightEdit
## Dev-mode-only Electric canopy LIGHTING tuning — group "electric_light_edit", toggled from
## arena_hud_buttons.gd's LIGHT EDIT button (Electric map only, sits directly above TERRAIN EDIT).
##
## Split out of ElectricTerrainEdit's own panel into its own small floating panel (same title-bar-drag pattern)
## so the canopy normal-map lighting knobs (tools/generate_canopy_normal.py's real per-pixel N.L + specular —
## see electric_ground.gdshader) have a focused home instead of living buried in the middle of the big terrain
## panel's slider list.
##
## Applies LIVE to the running ElectricGround instance (group "electric_ground"). SAVE persists to
## res://electric_terrain.cfg via ElectricTerrainSettings — loads/saves the FULL settings dict (not just these 4
...
func _ready() -> void:
	...
	add_to_group("electric_light_edit")
...
func _apply_live() -> void:
	...
	var sparks := get_tree().get_first_node_in_group("electric_sparks")
...
## reverted every OTHER panel's changes since (e.g. electric_terrain_edit.gd's) back to whatever they were when
...
func _on_reset() -> void:
	_values["canopy_light_angle_deg"] = ElectricTerrainSettings.DEFAULT_CANOPY_LIGHT_ANGLE_DEG
	_values["canopy_light_height"] = ElectricTerrainSettings.DEFAULT_CANOPY_LIGHT_HEIGHT
	_values["canopy_ambient"] = ElectricTerrainSettings.DEFAULT_CANOPY_AMBIENT
	_values["canopy_contrast"] = ElectricTerrainSettings.DEFAULT_CANOPY_CONTRAST
	_values["canopy_specular"] = ElectricTerrainSettings.DEFAULT_CANOPY_SPECULAR
	_values["canopy_light_color"] = ElectricTerrainSettings.DEFAULT_CANOPY_LIGHT_COLOR
	_values["spark_amount"] = ElectricTerrainSettings.DEFAULT_SPARK_AMOUNT
	_values["spark_color"] = ElectricTerrainSettings.DEFAULT_SPARK_COLOR
	_values["spark_speed"] = ElectricTerrainSettings.DEFAULT_SPARK_SPEED
	_values["spark_size"] = ElectricTerrainSettings.DEFAULT_SPARK_SIZE
	_values["spark_direction_deg"] = ElectricTerrainSettings.DEFAULT_SPARK_DIRECTION_DEG
	_values["spark_brightness"] = ElectricTerrainSettings.DEFAULT_SPARK_BRIGHTNESS
	_values["spark_opacity"] = ElectricTerrainSettings.DEFAULT_SPARK_OPACITY
	_sync_ui_from_values()
	_apply_live()
	if _status != null:
