extends CanvasLayer
class_name MechanicLightEdit
## Dev-mode-only Mechanic canopy LIGHTING tuning — group "mechanic_light_edit", toggled from
## arena_hud_buttons.gd's LIGHT EDIT button (Mechanic map only, sits directly above TERRAIN EDIT).
##
## Split out of MechanicTerrainEdit's own panel into its own small floating panel (same title-bar-drag pattern)
## so the canopy normal-map lighting knobs (tools/generate_canopy_normal.py's real per-pixel N.L + specular —
## see mechanic_ground.gdshader) have a focused home instead of living buried in the middle of the big terrain
## panel's slider list.
##
## Applies LIVE to the running MechanicGround instance (group "mechanic_ground"). SAVE persists to
## res://mechanic_terrain.cfg via MechanicTerrainSettings — loads/saves the FULL settings dict (not just these 4
...
func _ready() -> void:
	layer = 61   # same tier as mechanic_terrain_edit.gd / creep_info_panel.gd
	process_mode = Node.PROCESS_MODE_ALWAYS
	add_to_group("mechanic_light_edit")
...
func _apply_live() -> void:
	...
	var sparks := get_tree().get_first_node_in_group("mechanic_sparks")
...
## opened). 2026-08-08 bug fix (found via the Atlantic port — the very "Mechanic map" recurrence the user
## flagged): with the old blanket save, opening this panel and clicking SAVE at any later point silently
## reverted every OTHER panel's changes since (e.g. mechanic_terrain_edit.gd's) back to whatever they were when
...
func _on_reset() -> void:
	_values["canopy_light_angle_deg"] = MechanicTerrainSettings.DEFAULT_CANOPY_LIGHT_ANGLE_DEG
	_values["canopy_light_height"] = MechanicTerrainSettings.DEFAULT_CANOPY_LIGHT_HEIGHT
	_values["canopy_ambient"] = MechanicTerrainSettings.DEFAULT_CANOPY_AMBIENT
	_values["canopy_contrast"] = MechanicTerrainSettings.DEFAULT_CANOPY_CONTRAST
	_values["canopy_specular"] = MechanicTerrainSettings.DEFAULT_CANOPY_SPECULAR
	_values["canopy_light_color"] = MechanicTerrainSettings.DEFAULT_CANOPY_LIGHT_COLOR
	_values["spark_amount"] = MechanicTerrainSettings.DEFAULT_SPARK_AMOUNT
	_values["spark_color"] = MechanicTerrainSettings.DEFAULT_SPARK_COLOR
	_values["spark_speed"] = MechanicTerrainSettings.DEFAULT_SPARK_SPEED
	_values["spark_size"] = MechanicTerrainSettings.DEFAULT_SPARK_SIZE
	_values["spark_direction_deg"] = MechanicTerrainSettings.DEFAULT_SPARK_DIRECTION_DEG
	_values["spark_brightness"] = MechanicTerrainSettings.DEFAULT_SPARK_BRIGHTNESS
	_values["spark_opacity"] = MechanicTerrainSettings.DEFAULT_SPARK_OPACITY
	_sync_ui_from_values()
	_apply_live()
	if _status != null:
