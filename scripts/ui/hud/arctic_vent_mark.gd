extends CanvasLayer
class_name ArcticVentMark
## Dev-mode-only Arctic vent-marking tool — group "arctic_vent_mark", toggled from arena_hud_buttons.gd's
## CRATER MARK button (Arctic map only; button/group name kept generic across maps). Port of
## mechanic_vent_mark.gd (see that file's header for the full rationale — shows the maptile set's reference
...
## arctic_plumes.gd).
##
## NO TABS — one vent kind (energy beam), so there's just one flat mark list (`beam_marks`). Photo thumbnail
## row is built from HOWEVER MANY images the current maptile set actually has (2026-08-19, on request —
## "hệ thống blend dynamic tự động phát hiện"), not Mechanic's fixed 7 — labels are generated A, B, C...
## positionally instead of a hardcoded 7-entry list.
##
## Left-click on the photo = add a mark. Right-click near an existing mark = remove it. Applies LIVE to the
## running ArcticPlumes instance (apply_vent_marks) on every add/remove. SAVE persists to
## res://arctic_terrain.cfg via ArcticTerrainSettings — loads/saves the FULL settings dict so it never
...
const DOT_COLOR := Color(0.45, 0.85, 0.95, 0.9)   # icy-cyan mark dot
...
var _tex_paths: Array = []       # positional, from ArcticAssetScan.maptile_set_image_paths()
...
func toggle() -> void:
	...
	if _panel == null:
		...
		_tex_paths = ArcticAssetScan.maptile_set_image_paths(String(_values["maptile_set"]))
		_build_ui()
	_select_tex(0)
...
## Positional label for photo index `i` — "A".."Z" then "AA".."AZ"... (never expected to matter past a
## handful of photos, but doesn't hard-cap at 26 either).
func _label_for(i: int) -> String:
	var s := ""
	var n := i
	while true:
		s = String.chr(65 + (n % 26)) + s
		n = n / 26 - 1
		if n < 0:
			break
	return s
...
func _build_ui() -> void:
	...
	sb.bg_color = Color(0.06, 0.08, 0.09, 0.98)
	sb.set_corner_radius_all(10)
	sb.set_border_width_all(2)
	sb.border_color = Color(0.35, 0.75, 0.85)   # icy-cyan border — distinct from the other Arctic panels
	...
	for i in _tex_paths.size():
		...
		btn.tooltip_text = "Photo %s" % _label_for(i)
	...
	# Square display at IMAGE_SIZE — source photos are themselves square, so uniform STRETCH_SCALE here never
	# distorts aspect ratio, just downsizes.
...
## for why (minifying the raw source directly reads as noisy static at THUMB_SIZE).
...
## Marking a vent is a discrete, deliberate click — applies live AND writes to disk immediately (mirrors
## mechanic_vent_mark.gd's own convention). Saves ONLY the "beam_marks" key, re-read against whatever's
## CURRENTLY on disk rather than this panel's own `_values` snapshot, so it never clobbers an unsaved live
## edit made meanwhile in Terrain/Light/Plume Edit.
func _commit() -> void:
	_push_live()
	_save_marks_only()
	if _status != null:
