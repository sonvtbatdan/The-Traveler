...
## Dev-mode-only Rubicon terrain tuning — group "rubicon_terrain_edit", toggled from
## arena_hud_buttons.gd's TERRAIN EDIT button (Rubicon map only, sits above the Simplified button).
##
...
##   • ASSETS panel — one icon per discovered .glb (RubiconAssetScan), baked from its own top-down thumbnail
##     (tools/bake_rubicon_trees.gd). Click = select for editing; Shift+click = ADD to the selection (multi-
...
##
## Applies LIVE to the running RubiconGround / RubiconClouds / RubiconTrees instances (found via their own
## groups — "rubicon_ground"/"rubicon_clouds"/"rubicon_trees"). SAVE persists to res://rubicon_terrain.cfg
## via RubiconTerrainSettings so the look survives the next session (mirrors creep_info_panel.gd's
...
func _build_terrain_panel() -> void:
	...
	_add_dropdown(col, "water_tile_set", "Water Pattern", RubiconAssetScan.watertile_set_names())
	_add_slider(col, "water_wave_size", "Wave Size", 60.0, 800.0, 10.0)
	_add_color_row(col, "river_bank_color", "River Bank Color (sand)")   # also colors the landmark ring — see rubicon_ground.gdshader
	_add_slider(col, "landmark_ring_width", "Landmark Ring Width", 0.0, 120.0, 2.0)
	_add_slider(col, "jitter", "Jitter", 0.0, 1.0, 0.02)
	_add_dropdown(col, "maptile_set", "Tile Set", RubiconAssetScan.maptile_set_names())
...
## Baked top-down reference thumbnail (tools/bake_rubicon_trees.gd) if one exists next to the .glb, else
## null (caller falls back to a text label).
func _load_asset_icon(glb_path: String) -> Texture2D:
	var png_path := RubiconAssetScan.baked_png_path(glb_path)
...
func _on_asset_enabled_toggled(on: bool, type_name: String) -> void:
	...
	var entry: Dictionary = settings.get(type_name, RubiconTerrainSettings.default_asset_entry())
	entry["enabled"] = on
...
func _sync_asset_detail_from_values() -> void:
	...
	var entry: Dictionary = _values["asset_settings"].get(anchor, RubiconTerrainSettings.default_asset_entry())
...
## depth testing against the cloud occluder mesh (rubicon_trees.gd), so even a tall instance still has its
...
## Pushes ONE asset type's current settings to the running RubiconTrees instance — used for every selected
...
## captured once at panel-build time (mirrors _build_asset_panel's one-time RubiconAssetScan.glb_paths() scan;
...
func _on_reset() -> void:
	...
	_values["cloud_color"] = RubiconTerrainSettings.DEFAULT_CLOUD_COLOR
	_values["cloud_clumpiness"] = RubiconTerrainSettings.DEFAULT_CLOUD_CLUMPINESS
	_values["river_width"] = RubiconTerrainSettings.DEFAULT_RIVER_WIDTH
	_values["river_bank_color"] = RubiconTerrainSettings.DEFAULT_RIVER_BANK_COLOR
	_values["landmark_ring_width"] = RubiconTerrainSettings.DEFAULT_LANDMARK_RING_WIDTH
	_values["water_color"] = RubiconTerrainSettings.DEFAULT_WATER_COLOR
	_values["water_wave_size"] = RubiconTerrainSettings.DEFAULT_WATER_WAVE_SIZE
	_values["jitter"] = RubiconTerrainSettings.DEFAULT_JITTER
	_values["canopy_size"] = RubiconTerrainSettings.DEFAULT_CANOPY_SIZE
	_values["maptile_set"] = RubiconTerrainSettings.DEFAULT_MAPTILE_SET
	_values["water_tile_set"] = RubiconTerrainSettings.DEFAULT_WATER_TILE_SET
	_values["color_a"] = RubiconTerrainSettings.DEFAULT_COLOR_A
	_values["color_b"] = RubiconTerrainSettings.DEFAULT_COLOR_B
	for type_name: String in _selected_assets.keys():
		_values["asset_settings"][type_name] = RubiconTerrainSettings.default_asset_entry()
	_sync_ui_from_values()
	_apply_live_global()
	for type_name: String in _selected_assets.keys():
		_apply_live_for(type_name)
