extends CanvasLayer
class_name AtlanticCraterMark
## Dev-mode-only Atlantic vent-marking tool — group "atlantic_crater_mark", toggled from
## arena_hud_buttons.gd's CRATER MARK button (Atlantic map only). Port of
## scripts/ui/hud/volcanic_crater_mark.gd (see that file's header for the full rationale — shows the maptile
## set's reference photos so the user can click directly on a seabed vent/fissure they see IN THE PHOTO; a
## mark is a normalized (u, v) on ONE specific photo, replicated at EVERY world-space repetition of that photo
## by atlantic_clouds.gd, not a single world position). Retitled "VENT MARK" — Atlantic's ambient plume
## sources are seabed vents, not volcanic craters (see atlantic_clouds.gd's header terminology).
##
## TWO TABS — "BUBBLE" / "WHIRLPOOL" — same reference photos, two independent mark lists
## (`bubble_crater_marks`/`whirlpool_crater_marks`); a spot can be marked for one, the other, or both.
##
## Left-click on the photo = add a mark (active tab). Right-click near an existing mark (active tab) = remove
## it. Applies LIVE to the running AtlanticClouds instance (apply_crater_marks) on every add/remove. SAVE
## persists to res://atlantic_terrain.cfg via AtlanticTerrainSettings — loads/saves the FULL settings dict so
## it never clobbers Terrain/Light/Plume/Landmark Edit's own keys living in the same file.
...
const DOT_COLOR := {"bubble": Color(0.55, 0.9, 1.0, 0.9), "whirlpool": Color(0.15, 0.4, 0.85, 0.9)}
...
var _values: Dictionary = {}     # full settings dict — we mutate _values["bubble_crater_marks"]/["whirlpool_crater_marks"]
var _tex_paths: Array = []       # positional a/b/c, from AtlanticAssetScan.maptile_set_image_paths()
...
func _ready() -> void:
	...
	add_to_group("atlantic_crater_mark")
...
func toggle() -> void:
	...
	if _panel == null:
		...
		_tex_paths = AtlanticAssetScan.maptile_set_image_paths(String(_values["maptile_set"]))
		_build_ui()
	_select_kind("bubble")
	_select_tex(0)
...
func _build_ui() -> void:
	...
	_font(title, 15, Color(0.75, 0.9, 1.0))
	...
	_font(hint, 10, Color(0.55, 0.65, 0.7))
	...
	# The image area scrolls (both axes, auto) instead of the panel overflowing the screen — see
	# volcanic_crater_mark.gd's header for the full TextureRect-sizing rationale.
	...
	# EXPAND_IGNORE_SIZE is the fix for a TextureRect's minimum size otherwise tying to native texture
	# resolution regardless of stretch_mode — see volcanic_crater_mark.gd's header for the full explanation.
	...
	_font(_count_lbl, 12, Color(0.8, 0.88, 0.92))
...
## CPU-resized thumbnail for a maptile photo's Button icon — matches this project's established "resize the
## Image, don't rely on stretch_mode" convention (see CLAUDE.md's Image scaling rule).
...
func _make_dot() -> Control:
	...
	var sb := StyleBoxFlat.new()
	sb.bg_color = DOT_COLOR.get(_kind, Color(0.55, 0.9, 1.0, 0.9))
...
## Marking a vent is a discrete, deliberate click — same reasoning as volcanic_crater_mark.gd's _commit():
## every add/remove/undo/clear both applies live AND writes to disk immediately, scoped to ONLY the
## *_crater_marks keys (re-read fresh from disk first) so it never clobbers another panel's unsaved edit.
func _commit() -> void:
	_push_live()
	_save_marks_only()
	if _status != null:
