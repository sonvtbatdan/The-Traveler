extends CanvasLayer
class_name AtlanticTerrainEdit
## Dev-mode-only Atlantic terrain tuning — group "atlantic_terrain_edit", toggled from arena_hud_buttons.gd's
## TERRAIN EDIT button (Atlantic map only). Mirrors scripts/ui/hud/volcanic_terrain_edit.gd (same two-panel
## ASSETS + TERRAIN EDIT layout, same drag/multi-select asset-editing behavior). Deltas: reads/writes
## AtlanticAssetScan/AtlanticTerrainSettings/atlantic_terrain.cfg instead, labels reworded for the deep-sea
## palette (silt/sand/current instead of basalt/ash/lava).
...
func _new_panel_shell(pos: Vector2, w: float, max_h: float, title_text: String) -> Dictionary:
	...
	_font(title, 15, Color(0.85, 1.0, 0.95))
...
func _build_terrain_panel() -> void:
	...
	_add_slider(col, "river_width", "Current Width", 0.0, 0.15, 0.005)
	_add_slider(col, "river_bank_width", "Current Bank Width", 0.0, 0.05, 0.001)
	_add_slider(col, "river_edge_jaggedness", "Current Edge Scour", 0.0, 1.5, 0.02)
	_add_color_row(col, "water_color", "Current Color")
	_add_dropdown(col, "water_tile_set", "Current Pattern", AtlanticAssetScan.watertile_set_names())
	_add_slider(col, "water_wave_size", "Current Flow Size", 60.0, 800.0, 10.0)
	_add_slider(col, "water_wave_speed", "Current Flow Speed", 0.0, 60.0, 1.0)
	_add_color_row(col, "river_bank_color", "Sand Rim Color")
	col.add_child(HSeparator.new())
	var water_surface_lbl := Label.new()
	water_surface_lbl.text = "Water Surface (whole-screen)"
	_font(water_surface_lbl, 13, Color(0.7, 1.0, 0.9))
	col.add_child(water_surface_lbl)
	_add_slider(col, "water_distort_strength", "Ripple Distortion", 0.0, 0.02, 0.0005)
	_add_slider(col, "water_distort_speed", "Ripple Speed", 0.0, 0.3, 0.01)
	_add_slider(col, "water_sparkle_intensity", "Sparkle Intensity", 0.0, 1.5, 0.02)
	_add_slider(col, "water_sparkle_speed", "Sparkle Speed", 0.0, 0.4, 0.01)
	_add_color_row(col, "water_sparkle_color", "Sparkle Color")
	_add_slider(col, "jitter", "Jitter", 0.0, 1.0, 0.02)
	_add_dropdown(col, "maptile_set", "Tile Set", AtlanticAssetScan.maptile_set_names())
	_add_slider(col, "canopy_size", "Ground Tile Size", 400.0, 4000.0, 25.0)
	col.add_child(HSeparator.new())
	_add_color_row(col, "color_a", "Terrain Color A (dark silt)")
	_add_color_row(col, "color_b", "Terrain Color B (pale sand)")
	col.add_child(HSeparator.new())

	_selection_lbl = Label.new()
	_font(_selection_lbl, 13, Color(0.7, 1.0, 0.9))
	...
	_font(_height_lbl, 11, Color(0.6, 0.9, 0.9))
...
## assets/map/atlantic/ currently has no .glb files, so this returns "" and _build_asset_panel() itself
## returns without creating a panel (mirrors volcanic_terrain_edit.gd's empty-folder handling).
func _build_asset_panel() -> String:
	var glb_paths: Array = AtlanticAssetScan.glb_paths()
	...
	_font(hint, 10, Color(0.5, 0.65, 0.65))
	...
	for glb_path: String in glb_paths:
		var type_name := AtlanticAssetScan.type_name(glb_path)
		if first_name == "":
			first_name = type_name
		if not asset_settings.has(type_name):
			asset_settings[type_name] = AtlanticTerrainSettings.default_asset_entry()
...
func _load_asset_icon(glb_path: String) -> Texture2D:
	var png_path := AtlanticAssetScan.baked_png_path(glb_path)
...
func _on_asset_enabled_toggled(on: bool, type_name: String) -> void:
	...
	var entry: Dictionary = settings.get(type_name, AtlanticTerrainSettings.default_asset_entry())
	entry["enabled"] = on
...
func _sync_asset_detail_from_values() -> void:
	...
	var entry: Dictionary = _values["asset_settings"].get(anchor, AtlanticTerrainSettings.default_asset_entry())
...
func _update_height_label() -> void:
	...
	var h_min: float = AtlanticTreesScript.DESIRED_HEIGHT_PX * float(entry["scale_min"])
...
# ── Global sliders/colors (current + terrain color) ────────────────────────────────────────────────
...
func _apply_live_global() -> void:
	...
	var water_surface := get_tree().get_first_node_in_group("atlantic_water_surface")
	if water_surface != null and water_surface.has_method("apply_water_surface_settings"):
		water_surface.call("apply_water_surface_settings", _values["water_distort_strength"], _values["water_distort_speed"],
			_values["water_sparkle_intensity"], _values["water_sparkle_color"], _values["water_sparkle_speed"])
	var clouds := get_tree().get_first_node_in_group("atlantic_clouds")
	if clouds != null and clouds.has_method("apply_canopy_size"):
		clouds.call("apply_canopy_size", _values["canopy_size"])   # marked-vent replication period depends on it
	if clouds != null and clouds.has_method("apply_river_width"):
		clouds.call("apply_river_width", _values["river_width"])   # keeps bubble/whirlpool vents off the current band
	var trees := get_tree().get_first_node_in_group("atlantic_trees")
...
## registered slider/color-picker/dropdown keys, plus asset_settings), instead of blanket-writing this panel's
...
func _on_reset() -> void:
	_values["river_width"] = AtlanticTerrainSettings.DEFAULT_RIVER_WIDTH
	_values["river_bank_width"] = AtlanticTerrainSettings.DEFAULT_RIVER_BANK_WIDTH
	_values["river_edge_jaggedness"] = AtlanticTerrainSettings.DEFAULT_RIVER_EDGE_JAGGEDNESS
	_values["river_bank_color"] = AtlanticTerrainSettings.DEFAULT_RIVER_BANK_COLOR
	_values["water_color"] = AtlanticTerrainSettings.DEFAULT_WATER_COLOR
	_values["water_wave_size"] = AtlanticTerrainSettings.DEFAULT_WATER_WAVE_SIZE
	_values["water_wave_speed"] = AtlanticTerrainSettings.DEFAULT_WATER_WAVE_SPEED
	_values["water_distort_strength"] = AtlanticTerrainSettings.DEFAULT_WATER_DISTORT_STRENGTH
	_values["water_distort_speed"] = AtlanticTerrainSettings.DEFAULT_WATER_DISTORT_SPEED
	_values["water_sparkle_intensity"] = AtlanticTerrainSettings.DEFAULT_WATER_SPARKLE_INTENSITY
	_values["water_sparkle_color"] = AtlanticTerrainSettings.DEFAULT_WATER_SPARKLE_COLOR
	_values["water_sparkle_speed"] = AtlanticTerrainSettings.DEFAULT_WATER_SPARKLE_SPEED
	_values["jitter"] = AtlanticTerrainSettings.DEFAULT_JITTER
	_values["canopy_size"] = AtlanticTerrainSettings.DEFAULT_CANOPY_SIZE
	_values["maptile_set"] = AtlanticTerrainSettings.DEFAULT_MAPTILE_SET
	_values["water_tile_set"] = AtlanticTerrainSettings.DEFAULT_WATER_TILE_SET
	_values["color_a"] = AtlanticTerrainSettings.DEFAULT_COLOR_A
	_values["color_b"] = AtlanticTerrainSettings.DEFAULT_COLOR_B
	for type_name: String in _selected_assets.keys():
		_values["asset_settings"][type_name] = AtlanticTerrainSettings.default_asset_entry()
	_sync_ui_from_values()
	_apply_live_global()
	for type_name: String in _selected_assets.keys():
		_apply_live_for(type_name)
