extends CanvasLayer
class_name ArcticLightEdit
## Dev-mode-only Arctic canopy LIGHTING tuning — group "arctic_light_edit", toggled from
## arena_hud_buttons.gd's LIGHT EDIT button (Arctic map only). Port of mechanic_light_edit.gd (see that
## file's header for the full rationale — identical here).
##
## Applies LIVE to the running ArcticGround instance (group "arctic_ground"). SAVE persists to
## res://arctic_terrain.cfg via ArcticTerrainSettings — loads/saves the FULL settings dict (not just these 4
...
func _ready() -> void:
	layer = 61   # same tier as arctic_terrain_edit.gd / creep_info_panel.gd
	process_mode = Node.PROCESS_MODE_ALWAYS
	add_to_group("arctic_light_edit")
...
func _build_ui() -> void:
	...
	_font(title, 15, Color(1.0, 0.95, 0.85))
...
func _apply_live() -> void:
	...
	var sparks := get_tree().get_first_node_in_group("arctic_sparks")
...
func _on_reset() -> void:
	_values["canopy_light_angle_deg"] = ArcticTerrainSettings.DEFAULT_CANOPY_LIGHT_ANGLE_DEG
	_values["canopy_light_height"] = ArcticTerrainSettings.DEFAULT_CANOPY_LIGHT_HEIGHT
	_values["canopy_ambient"] = ArcticTerrainSettings.DEFAULT_CANOPY_AMBIENT
	_values["canopy_contrast"] = ArcticTerrainSettings.DEFAULT_CANOPY_CONTRAST
	_values["canopy_specular"] = ArcticTerrainSettings.DEFAULT_CANOPY_SPECULAR
	_values["canopy_light_color"] = ArcticTerrainSettings.DEFAULT_CANOPY_LIGHT_COLOR
	_values["spark_amount"] = ArcticTerrainSettings.DEFAULT_SPARK_AMOUNT
	_values["spark_color"] = ArcticTerrainSettings.DEFAULT_SPARK_COLOR
	_values["spark_speed"] = ArcticTerrainSettings.DEFAULT_SPARK_SPEED
	_values["spark_size"] = ArcticTerrainSettings.DEFAULT_SPARK_SIZE
	_values["spark_direction_deg"] = ArcticTerrainSettings.DEFAULT_SPARK_DIRECTION_DEG
	_values["spark_brightness"] = ArcticTerrainSettings.DEFAULT_SPARK_BRIGHTNESS
	_values["spark_opacity"] = ArcticTerrainSettings.DEFAULT_SPARK_OPACITY
	_sync_ui_from_values()
	_apply_live()
	if _status != null:
