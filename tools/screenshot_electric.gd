extends SceneTree
## One-shot DEV TOOL (not shipped/imported into gameplay): boots scenes/electric.tscn standalone (no hub,
...
##   godot --path . --script tools/screenshot_electric.gd
## Output: user://electric_shot_1_initial.png, user://electric_shot_2_moved.png
...
func _initialize() -> void:
	var packed := load("res://scenes/electric.tscn") as PackedScene
...
func _on_post_draw() -> void:
	...
	if _phase == 0 and _f >= SETTLE_FRAMES:
		_save("user://electric_shot_1_initial.png")
	...
	if _phase == 1 and _f >= MOVE_FRAMES:
		...
		_save("user://electric_shot_2_moved.png")
		_phase = 2
		_f = 0
		return
...
func _save(path: String) -> void:
	...
	if err == OK:
		print("screenshot_electric: saved ", path, "  ->  ", ProjectSettings.globalize_path(path))
	else:
		push_warning("screenshot_electric: save failed (%d) for %s" % [err, path])
