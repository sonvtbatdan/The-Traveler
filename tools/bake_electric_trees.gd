extends SceneTree
## One-shot THUMBNAIL bake: render each tree/temple .glb in assets/map/electric/ into a top-down reference
## PNG with real alpha. NOT used for in-game scattering any more — electric_trees.gd renders the live .glb
...
##   godot --path . --script tools/bake_electric_trees.gd
## Output: assets/map/electric/<model_name>.png, alongside each source .glb. Models are AUTO-DISCOVERED
## (ElectricAssetScan.glb_paths() — every .glb directly in assets/map/electric/) — just drop a new .glb in
...
func _initialize() -> void:
	...
	_queue = ElectricAssetScan.glb_paths()
	if _queue.is_empty():
		print("bake_electric_trees: no .glb found in ", ElectricAssetScan.FOLDER)
		quit(0)
		return
...
func _start_next() -> void:
	...
	if not ResourceLoader.exists(path):
		push_warning("bake_electric_trees: missing " + path)
		call_deferred("_start_next")
		return
	var packed := load(path) as PackedScene
	if packed == null:
		push_warning("bake_electric_trees: failed to load " + path)
		call_deferred("_start_next")
		return
...
func _on_post_draw() -> void:
	...
	if _rf == SETTLE_FRAMES:
		...
		if err == OK:
			print("bake_electric_trees: saved ", out_path)
		else:
			push_warning("bake_electric_trees: save failed (%d) for %s" % [err, out_path])
		call_deferred("_start_next")
