...
func _initialize() -> void:
	var packed := load("res://scenes/volcanic.tscn") as PackedScene
...
func _on_post_draw() -> void:
	...
	if _phase == 0 and _f >= SETTLE_FRAMES:
		_save("user://volcanic_shot_1_initial.png")
	...
	if _phase == 1 and _f >= MOVE_FRAMES:
		...
		_save("user://volcanic_shot_2_moved.png")
		_phase = 2
		_f = 0
		return
...
func _save(path: String) -> void:
	...
	if err == OK:
		print("screenshot_volcanic: saved ", path, "  ->  ", ProjectSettings.globalize_path(path))
	else:
		push_warning("screenshot_volcanic: save failed (%d) for %s" % [err, path])
