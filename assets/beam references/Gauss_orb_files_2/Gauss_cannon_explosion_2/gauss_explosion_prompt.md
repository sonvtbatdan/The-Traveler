...
THREE explosion variants, each 12 frames, ALL aligned to the SAME spec so they're interchangeable: **336×336 each, burst core centered at (168,168)**, additive, transparent PNG. The files are named by variant + frame index (e.g. `v1_00..v1_11`, `v2_00..v2_11`, `v3_00..v3_11`). In STAGE 0, list that folder and report the EXACT filenames found, so the importer references real names (don't assume — confirm the naming/case).
...
0. List the art folder `C:\Users\minhv\Documents\The-Stream\assets\beam references\Gauss_orb_files_2\Gauss_cannon_explosion` and report the EXACT filenames (so the importer uses real names/case). Confirm 3 variants × 12 frames, each 336×336 with alpha.
...
- Import frames to `res://assets/fx/gauss_explosion/v1|v2|v3/`, additive, core-anchored at (168,168). Pick a random variant per explosion (cosmetic only — same timing/radius/damage for all).
